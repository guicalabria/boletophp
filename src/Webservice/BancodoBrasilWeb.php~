...
class BancodoBrasilWeb {
   ...
   public $mensagem = '';
  	...
  	public function __construct(array $params) 
  	{
  	   ...
  	   {
  	      throw new \Exception('Client Secrect não configurado');
  	   }
  	   
  	   
  	   if ( isset($params['arquivoteste']) && $params['arquivoteste']==1  ) $this->alterarParaAmbienteDeTestes();
  	 ...
  	 */
  	public private function _prepararCurl() 
  	{
    		$curl = curl_init();
  	...
  	public function obterToken($naousarcache = true) 
  	{
    		...
    		if ( !is_dir($this->_caminhoPastaCache) )
    		{
    		   ...
    		   {
    		      $this->erro = true;
    		      $this->mensagem .= 'Erro ao gerar diretório cache';
    		      return false;
    		   }
    		}
    		...
    		if ($resultado) 
    		{
     			if (isset($resultado->access_token)) 
     			{
     			 ...
     			 {
     			    $this->erro = true;
     			    $this->mensagem = @$resultado->error_description ?: 'Erro inesperado na resposta do Banco do Brasil';
  		        return false;
     			 }
   		 }
   		 else
     	{
     			 $this->erro = true;
     			 $this->mensagem .= 'Não foi possível conectar-se ao Banco do Brasil';
  		     return false;
      }
  		  return false;
  	}
  	...
  	public function registrarBoleto($parametros, $token = false) 
  	{
         ...
         //file_put_contents('./requisicao.xml',$requisicao);
      			...
      			if ( $resposta )
      			{
        				...
        				{
        				 //file_put_contents('./reposta.xml',$resposta);
      			...
      			{
      			   $this->erro = true;
      				  $this->menasgem = is_array($resultado) ? @$resultado['detail']['erro']['Mensagem'] ?: @$resultado['textoMensagemErro'] : 'Erro inesperado na resposta do Banco do Brasil';
