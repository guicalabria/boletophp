...
class BB {
	...
	static private $_urls = array(
		...
		self::AMBIENTE_TESTE => array(
			...
			//'registro' => 'https://cobranca.homologa.bb.com.br:7101/Processos/Ws/RegistroCobrancaService.serviceagent?wsdl'
		)
	);
	...
	function obterToken($naousarcache = true) {
		...
		if ($resultado) 
		{
  			if (isset($resultado->access_token)) 
  			{
  			 ...
  			 else $this->_erro = @$resultado->error_description ?: 'Erro inesperado na resposta do Banco do Brasil';
		 }
		 else $this->_erro = 'Não foi possível conectar-se ao Banco do Brasil';
	...
	function registrarBoleto($parametros, $token = false) {
		...
		for (;;) 
		{
  			...
  			if (!$tokeninformada || $forcarobtertoken)
  			{
    				...
    				if ( !$token ) { $this->_erro = 'Erro ao obter a token do Banco do Brasil - ' . $this->_erro;return false; }
